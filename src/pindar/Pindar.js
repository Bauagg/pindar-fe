...
const Pindar = () => {
  ...
  const handleNextPage = () => {
    const page = offset + limit < totalData ? offset + limit : offset;
    setOffset(page);
  ...
  const handlePrevPage = () => {
    const page = Math.max(offset - limit, 0);
    setOffset(page);
    sessionStorage.setItem("current_page", page);
    fetchdatas(page);
  };

  const fetchdatas = async (page, key = "") => {
    try {
      const response = await api.get(
        `lender/list?limit=${limit}&offset=${page}&search=${key}&sortBy=maxloan&sortDirection=${selectedPlafond}&loanType=${selectedLoanAmounts}&paymentType=${selectedPayments}`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    // Ketika reload → hapus "current_page"
  ...
  const isAllLoanAmountsSelected =
    selectedLoanAmounts.length === loanAmountData.length;
