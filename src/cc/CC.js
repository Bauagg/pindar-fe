...
const DataCC = () => {
    ...
    const fetchdatas = async () => {
      try {
        const response = await api.get(`credit-card/search?featureId=${selectedFeature}&minYearlyFee=${yearlyFeeRange[0]}&maxYearlyFee=${yearlyFeeRange[1]}&sortBy=yearly_fee&sortDirection=${selectedPlafond}&limit=${limit}&offset=${offset}`, {
          headers: {
    ...
    const handleSearch = (e) => {
      ...
      const isAllPenerbitsSelected = selectedPenerbit.length === dataPenerbit.length;
      ...
      useEffect(() => {
        ...
        fetchfeatures();
        fetchPenerbit();
      }, [limit, offset, search, selectedFeature, selectedPenerbit, selectedPlafond, yearlyFeeRange]);
      ...
      const handleAjukan = (item) => {
        ...
        navigate("/ccajukan"); // pastikan useNavigate dari react-router-dom
      };
                            ...
                            <div className="mt-4">
                              ...
                              <div className="d-flex gap-2 mb-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setYearlyFeeRange([Number(e.target.value), yearlyFeeRange[1]])
                                  }
                                />
                                <input
                                  type="number"
                                  ...
                                  onChange={(e) =>
                                    setYearlyFeeRange([yearlyFeeRange[0], Number(e.target.value)])
                                  }
                                />
                              </div>
                              <ReactSlider
                        ...
                        <div key={item.id} className="card-pindar mb-3">
                            <div className="row">
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-sm-4">
                                    <label><b>Fitur</b></label>
                                      <ul className="list-unstyled">
                                      {item.features.map((itm, index) => (
                                          <li key={index}><i className="bi bi-check-circle-fill" style={{color: '#34C759'}}></i> {itm.feature}</li>
                                        ))}
                                      </ul>
                                    </div>
                        ...
                        <div className="col-sm-4">
                            <div className="d-flex mt-2">
                                ...
                                >
                                    ...
                                    />
                                    <span className="mx-2"><b>{item.title}</b></span>
                                </div>
                                ))}
                            </div>
                        </div>
